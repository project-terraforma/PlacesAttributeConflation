...
The dashboard is intentionally self-contained: it embeds a replay corpus JSON
payload into one HTML file and lets reviewers relabel/export the corpus without
...
def build_review_dashboard_html(episodes: Iterable[ReplayEpisode], *, title: str = "Replay Review Dashboard") -> str:
    episode_payload = [episode.to_dict() for episode in episodes]
    labels_payload = {
...
<head>
  ...
  <style>
    ...
    .page {{ border-left: 4px solid color-mix(in srgb, CanvasText 30%, transparent); padding: 10px; background: color-mix(in srgb, CanvasText 5%, transparent); border-radius: 8px; }}
    .muted {{ opacity: 0.72; }}
...
<script id=\"replay-data\" type=\"application/json\">{_json_for_html(episode_payload)}</script>
<script id=\"label-data\" type=\"application/json\">{_json_for_html(labels_payload)}</script>
<script>
...
function pagesFor(ep) {{
  return (ep.search_attempts || []).flatMap(a => (a.fetched_pages || []).map(p => ({{...p, layer: a.layer, query: a.query}})));
}}
function renderShortcuts() {{
  document.getElementById('shortcuts').innerHTML = Object.entries(labelData.shortcuts)
    .map(([k,v]) => `<div><span class=\"kbd\">${escapeHtml(k === ' ' ? 'Space' : k)}</span> ${escapeHtml(v)}</div>`).join('');
}}
...
function renderCase() {{
  ...
  document.getElementById('progress').textContent = `${idx + 1} / ${episodes.length} cases`;
  document.getElementById('case').innerHTML = `
    <div class=\"card\">
      <h2>${escapeHtml(ep.case_id)} <span class=\"muted\">${escapeHtml(ep.attribute)}</span></h2>
      <div class=\"grid\">
        ${field('Gold value', ep.gold_value)}
        ${field('Expected decision', ep.expected_decision)}
        ${field('Expected abstain', ep.expected_abstain)}
        ${field('Review status', ep.review_status || 'unreviewed')}
        ${field('Current value', ep.place?.current_value)}
        ${field('Base value', ep.place?.base_value)}
        ${field('Name', ep.place?.name)}
        ${field('Address', ep.place?.address)}
        ${field('City / region', [ep.place?.city, ep.place?.region].filter(Boolean).join(', '))}
        ${field('Overture id', ep.place?.overture_id || ep.place?.gers_id)}
      </div>
    </div>
    <div class=\"card\">
      <h3>Labels</h3>
      <div class=\"grid\">
        ${field('Case type', ep.case_type)}
        ${selectField('Website label', 'website_label', labelData.website_labels)}
        ${selectField('Identity label', 'identity_label', labelData.identity_labels)}
        ${field('Truth source type', ep.truth_source_type)}
        ${field('Label origin', ep.label_origin)}
        ${field('Difficulty', ep.difficulty)}
      </div>
      <p><label><input type=\"checkbox\" ${ep.expected_abstain === true ? 'checked' : ''} onchange=\"setField('expected_abstain', this.checked)\" /> Expected abstain</label></p>
      <textarea placeholder=\"Reviewer notes\" oninput=\"setField('reviewer_notes', this.value)\">${escapeHtml(ep.reviewer_notes || '')}</textarea>
    </div>
    <div class=\"card\">
      <h3>Evidence pages (${pages.length})</h3>
      <div class=\"pages\">${pages.map(p => `
        <div class=\"page\">
          <div><a href=\"${escapeHtml(p.url)}\" target=\"_blank\" rel=\"noreferrer\">${escapeHtml(p.title || p.url)}</a></div>
          <div class=\"muted\">${escapeHtml(p.source_type)} · ${escapeHtml(p.evidence_role)} · ${escapeHtml(p.source_family_id)} · ${escapeHtml(p.layer)}</div>
          <div class=\"muted\">Query: ${escapeHtml(p.query)}</div>
          <p>${escapeHtml((p.page_text || '').slice(0, 900))}</p>
        </div>`).join('')}</div>
    </div>`;
}}
...
function downloadReviewed() {{
  const payload = {{schema_version: 1, generated_at: new Date().toISOString(), episodes}};
  const blob = new Blob([JSON.stringify(payload, null, 2)], {{type: 'application/json'}});
  const a = document.createElement('a');
  a.href = URL.createObjectURL(blob);
  a.download = 'reviewed_replay.json';
  a.click();
}}
document.addEventListener('keydown', ev => {{
  if (['INPUT','TEXTAREA','SELECT'].includes(document.activeElement.tagName)) return;
  const ep = episodes[idx];
  if (ev.key === 'ArrowLeft') {{ mark('rejected'); ev.preventDefault(); }}
  if (ev.key === 'ArrowRight') {{ mark('accepted'); ev.preventDefault(); }}
  if (ev.key === 'Enter') {{ nextCase(); ev.preventDefault(); }}
  if (ev.key === ' ') {{ mark('skipped'); nextCase(); ev.preventDefault(); }}
